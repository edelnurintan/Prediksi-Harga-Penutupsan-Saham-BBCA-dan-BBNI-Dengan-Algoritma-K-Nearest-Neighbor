...
# %%
#import dataset
df = pd.read_csv(r'D:\skripsi\BBNI\BBNI.csv')
bbni_belum = df.head()
bbni_belum.to_csv("Dataset BBNI.csv",index=False)

# %%
saham = df[['Date','Open', 'High', 'Low', 'Close']]
saham1 = saham.head()
saham1.to_csv("data_bbni.csv")
...
for k, rmse in rmse_list:
   if k == 3 :
      rmse_KNN=rmse # mengambil RMSE untuk K = 8 yang telah dihitung
...
plt.plot(indeks, y_pred, label='Prediksi', linestyle='-', color='red')
...
# %%
data_ara = pd.read_csv(r'D:\skripsi\BBNI\BBNI15-17.csv')
...
print(bbca_ara)
bbca_ara[['Date','Open','High','Low','Close']].to_csv('bbni_ara_arb.csv',index=False)
...
# %%
BBCAara_arb = pd.read_csv(r'D:\skripsi\BBNI\bbni_ara_arb.csv')
high_low = BBCAara_arb[['Date','Open','High','Low']]
high_low
...
prediksi = knn_regression(X_train, Y_train, bbca_arr,3)
...
hasil.to_csv('hasilPrediksiARAdanARBBNI.csv', index=False)
print(hasil)
...
def prediksimenit16(x,y,x_train,y_train,data_n):
    data = ambil_data(x,y,data_n)
    data = data[['Open','High','Low']]
    max_high = data['High'].max()
    min_low = data['Low'].min()
    #max_avg = data['High'].mean()
    #min_avg = data['Low'].mean()
    open16 = data['Open'][x]
    arr = [[open16,max_high,min_low]]
    arr = np.array(arr)
    prediction_minmax = knn_regression(x_train,y_train,arr,3)
    strs1 = print(f'Hasil Prediksi Menggunakan High Tertinggi, dan Low Terendah : {prediction_minmax[0]} ')
    #arr_avg = [[open16, max_avg, min_avg]]
    #arr_avg = np.array(arr_avg)
   # prediction_avg = knn_regression(x_train,y_train,arr_avg,6)
    #strs2 = print(f'Hasil Prediksi Menggunakan moving avg: {prediction_avg[0]} ')
    return strs1
...
# %%
data_15 = pd.read_csv(r'D:\skripsi\BBNI\menit\BBNI_15.csv')

#prediksi tanggal 15 jam 10
print('Prediksi Jam 10 Tanggal 15')
...
# %%
data_15 = pd.read_csv(r'D:\skripsi\BBNI\menit\BBNI_16.csv')
...
# %%
data_17 = pd.read_csv(r'D:\skripsi\BBNI\menit\BBNI_17.csv')
...
# %%
data_15 = pd.read_csv(r'D:\skripsi\BBNI\menit\BBNI_20.csv')
...
# %%
data_15 = pd.read_csv(r'D:\skripsi\BBNI\menit\BBNI_21.csv')
